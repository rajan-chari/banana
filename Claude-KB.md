...
- **2026-04-08:** PowerShell 5.1 (ships with Windows Server 2022) cannot parse nested `@(@{...})` hashtable literals or JSON here-strings with colons. setup.ps1 requires pwsh 7. Install via `choco install powershell-core -y`.
...
- **2026-04-14:** ESM modules cannot use `require()` — crashes at runtime. Always use top-level `import` for Node built-ins (http, https, fs, etc.) in ESM TypeScript projects. This bit us in fellow-agents CLI (services.ts used require('http') inside a function).
...
- **2026-04-14:** xterm.js Ctrl+V paste fires BOTH a custom `attachCustomKeyEventHandler` AND `onData` — causing double paste. Fix: set a `_pasteGuard` flag in the Ctrl+V handler, check it in onData, clear after 50ms. Don't remove the custom handler entirely — xterm.js doesn't handle Ctrl+V paste by default in all configs. Right-click paste only fires onData (no guard needed).
