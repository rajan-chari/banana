...
/**
 * @typedef {Object} DashboardSessionsPort
 * @property {() => any[]} list  array of SessionInfo values
 * @property {() => Array<[string, any]>} entries  [name, info] pairs
 ...
 * @property {{ activeWorkspaceId?: string | null, isDashboard?: boolean }} state
 * @property {DashboardSessionsPort} sessions
...
export function createDashboardPanel(deps) {
  ...
  function buildHeaderHTML() {
    ...
    const alive = list.filter((i) => i.status !== "dead").length;
    const busy = list.filter((i) => i.status === "busy").length;
    return `
      ...
      <span class="dash-summary">
        ...
        <span class="val">${deps.sessions.size()}</span> total
  ...
  function patchDashboard(dash) {
    ...
    if (countEl) countEl.textContent = `(${deps.sessions.size()})`;

    const cardsGrid = dash.querySelector(".dash-cards");
    if (!cardsGrid) return;

    removeStaleCards(cardsGrid, new Set(deps.sessions.names()));

    for (const [name, info] of deps.sessions.entries()) {
  ...
  function render() {
    ...
    cardsHeader.innerHTML = `<span class="dash-cards-arrow">${cardsCollapsed ? "\u25b8" : "\u25be"}</span> Workspaces <span class="dash-cards-count">(${deps.sessions.size()})</span>`;
    cardsHeader.onclick = () => {
